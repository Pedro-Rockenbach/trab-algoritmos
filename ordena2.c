...
typedef struct {
	Tnum key; // chave de ordenação
	Tnum seq; // número de sequência de inserção no conjunto
} Tdado;
...
void escreve(Tdado V[], Tnum n) {
	printf("[");
	for (Tnum i = 0; i < n; i++) {
		printf("(%lld, %lld)", V[i].key, V[i].seq);
		if (i < n - 1) {
			printf(", ");
      }
	}
...
Tnum binSearch(Tnum elem, Tdado dados[], Tnum n) {
   Tnum lo = 0;
   Tnum hi  = n - 1;
   Tnum i = 0;
